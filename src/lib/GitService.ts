...
class GitService {
  ...
  private _gun = Gun(['https://gun-manhattan.herokuapp.com/gun']);
  private _peersNode = this._gun.get('haven-sovereign-git').get('peers');
  ...
  private setupP2PListeners() {
    this._peersNode.map().on((data: any, key: string) => {
      if (data && data.head && key !== this._author.email) {
        console.log(`[GitService] Peer Update from ${data.alias} (${key}): Branch: ${data.branch} HEAD: ${data.head}`);
        // Simulate an auto-fetch and notify UI
        this.emit();
      }
    });
  }

  public async syncP2P() {
    console.log('[GitService] Triggering Sovereign P2P Sync...');
    await this.broadcastHead();
    // Simulate fetching from peers
    return true;
  }

  private async broadcastHead() {
    try {
      const head = await git.resolveRef({ fs, dir: this._dir, ref: 'HEAD' });
      const branch = await this.currentBranch();
      this._peersNode.get(this._author.email).put({
        head,
        branch,
        timestamp: Date.now(),
        alias: this._author.name,
        email: this._author.email
      });
      console.log(`[GitService] Broadcasted HEAD (${branch}): ${head}`);
    } catch (err) {
      console.warn('[GitService] Could not broadcast HEAD:', err);
    }
  }
