 ...
 */
const webrtcHttpClient = (async ({ url, method, headers, body }: any) => {
    ...
    const responseBody = (async function* () {
        // Placeholder for P2P binary request
...
export const KForge = () => {
    ...
    const { connectedPeers, connectToPeer, broadcastUpdate, subscribeToUpdates } = useKForgeP2P();
    const { initRepo } = useKForgeGit();

    // Subscribe to all repos for P2P updates
    useEffect(() => {
        const repoSubscriptions = repos.map(repo => {
            subscribeToUpdates(repo.name, (sha) => {
                console.log(`[P2P-Sync] Received update for ${repo.name}: ${sha}`);
                setRepos(prev => prev.map(r => {
                    if (r.name === repo.name && r.status !== 'synced') {
                        return { ...r, status: 'synced', size: 'Remote SHA: ' + sha.slice(0, 7) };
                    }
                    return r;
                }));
            });
        ...
        return () => {
             // Cleanup if needed (GunDB's .off() isn't standard in all wrappers but good to keep in mind)
        };
    }, [repos.length]); // Re-subscribe if repo list changes
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
                {activeTab === 'repos' ? (
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="text-[10px] text-white/40 px-4 mt-6 border-t border-white/5 pt-4">
                            Transport Layer: <strong>WebRTC DataChannels</strong><br/>
                            Discovery Layer: <strong>GunDB (Khash/DHT)</strong><br/>
